...
def create_extra_val_loader(args, dataset, val_input_transform, target_transform, val_sampler):
    ...
    if dataset == 'cityscapes':
        ...
        val_set = gtav.GTAV('val', 0,
                            joint_transform=joint_transforms.ResizeHW(h=1052, w=1914),
                            transform=val_input_transform,
                            target_transform=target_transform,
                            cv_split=args.cv,
                            image_in=args.image_in)
                            # GTAV本來就是這個大小,幹嘛resize
    ...
    # from datasets.sampler import DistributedSampler
    # val_sampler = DistributedSampler(val_set, pad=False, permutation=True, consecutive_sample=False)
...
def setup_loaders(args):
    ...
    if 'gtav' in args.dataset:
        ...
        val_set = gtav.GTAV('train', 0,
                            joint_transform=joint_transforms.Resize(args.crop_size),
                            transform=val_input_transform,
                            target_transform=target_transform,
                            cv_split=args.cv,
                            image_in=args.image_in)
    ...
    for i, val_set in enumerate(val_sets):
        ...
        val_loader = DataLoader(val_set, batch_size=args.train_batch_size,
                                num_workers=args.num_workers // 2 , shuffle=True, drop_last=False)
    ...
    for val_dataset in args.val_dataset:
        extra_val_loader[val_dataset] = create_extra_val_loader(args, val_dataset, val_input_transform, target_transform, val_sampler=None)
