...
def add_items(items, aug_items, cities, img_path, mask_path, mask_postfix, mode, maxSkip):
    ...
    """
    # no augmented dataset (maybe from RobustNet?)
    for it in cities:
...
class GTAV(data.Dataset):
    ...
    def __init__(self, mode, maxSkip=0, joint_transform=None, sliding_crop=None,
                 ...
                 extract_feature=False, combine_all=False):
        self.mode = mode
        ...
        if cv_split:
            ...
            self.cv_split = 0
        self.imgs, _ = make_dataset(mode, self.maxSkip, cv_split=self.cv_split, combine_all=combine_all)
        if len(self.imgs) == 0:
    ...
    def __getitem__(self, index):
        ...
        for k, v in color_to_trainid.items():
            if v != 255 and v != -1:
                ...
                mask_copy[(mask[:, :, :3] == np.array(k))[:, :, 0] & (mask[:, :, :3] == np.array(k))[:, :, 1] & (mask[:, :, :3] == np.array(k))[:, :, 2]] = v
...
class GTAVUniform(data.Dataset):
    ...
    def __init__(self, mode, maxSkip=0, joint_transform_list=None, sliding_crop=None,
        ...
        self.imgs, self.aug_imgs = make_dataset(mode, self.maxSkip, cv_split=self.cv_split, combine_all=combine_all)
        ...
        if combine_all:
            json_fn = 'gtav_{}_cv{}_tile{}_all.json'.format(
            self.mode, self.cv_split, self.class_uniform_tile)
